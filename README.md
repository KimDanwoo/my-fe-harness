...
[![CI](https://github.com/danwoo/my-fe-harness/actions/workflows/ci.yml/badge.svg)](https://github.com/danwoo/my-fe-harness/actions/workflows/ci.yml)
...
/plugin marketplace add danwoo/my-fe-harness
/plugin install my-fe-harness@my-fe-harness

# 또는 설치 없이 npx — 권장 린 세트(코어 + 프레임워크)
npx github:danwoo/my-fe-harness add core react
```
...
npx github:danwoo/my-fe-harness init

# 권장(린): 코어 + 프레임워크 — 깔끔한 스타일에 필요한 최소
npx github:danwoo/my-fe-harness add core react
# 필요할 때 옵션 얹기
npx github:danwoo/my-fe-harness add a11y styling
npx github:danwoo/my-fe-harness add all                # 코어+옵션 전부 (무거움, 구조 제외)

# 전역 설치 — 모든 프로젝트에 적용 (~/.claude)
npx github:danwoo/my-fe-harness add core --global      # 규칙을 전역으로
npx github:danwoo/my-fe-harness guard --global         # 안전장치 훅을 전역으로(항상 켜둠)

# 폴더 구조 스캐폴드 + 해당 규칙 설치
npx github:danwoo/my-fe-harness scaffold feature-based
npx github:danwoo/my-fe-harness scaffold               # 대화형 선택

# 팩·구조 목록 · 버전
npx github:danwoo/my-fe-harness list
npx github:danwoo/my-fe-harness --version
```
...
npx github:danwoo/my-fe-harness guard
```
...
├── commands/              # 플러그인 슬래시 커맨드 (/install, /scaffold, /guard, /list)
...
- `test/cli` — `add`/`scaffold`/`guard`/`init`/`--global`/자동 감지 (임시 디렉토리, 실제 `~` 미변경).
